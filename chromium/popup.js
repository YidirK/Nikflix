...
document.getElementById("bug-btn").addEventListener("click", () => {
  chrome.tabs.create({
    url: "https://github.com/YidirK/Nikflix/issues/new?template=bug_report.md",
  });
});
...
async function getData() {
  const url =
    "https://raw.githubusercontent.com/YidirK/Nikflix/refs/heads/master/chromium/manifest.json";
  try {
...
toggle.addEventListener("change", function () {
  ...
  statusText.className = this.checked
...
document.addEventListener("DOMContentLoaded", function () {
  chrome.storage.local.get(["status"], function (result) {
    ...
    statusText.className = toggle.checked
      ? "status-text status-active"
      : "status-text status-inactive";
  });
});
